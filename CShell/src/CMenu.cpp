...
std::map<std::string, long> &CMenu::GetStandardCommand()
{
    ...
    while(!childNode.isNull())
    {
        ...
        if (tagName.compare("MENUITEM") == 0)
        {
            ...
            if (!idStr.isEmpty())
            {
                ...
                std::map<std::string, long> &menu_id_map = GetStandardCommand();

                it = menu_id_map.find(idStr.toStdString());

                if (it != menu_id_map.end())
                {
